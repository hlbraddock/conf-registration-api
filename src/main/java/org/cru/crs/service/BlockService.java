...
public class BlockService
{
	...
	public void updateBlock(BlockEntity blockToUpdate, UUID crsAppUserId) throws UnauthorizedException
	{
		verifyUserIdHasAccessToModifyThisBlocksConference(blockToUpdate,crsAppUserId);
		
		em.merge(blockToUpdate);
	}

	public void deleteBlock(UUID blockId, UUID crsAppUserId) throws UnauthorizedException
	{
		BlockEntity blockToDelete = em.find(BlockEntity.class, blockId);
		
		verifyUserIdHasAccessToModifyThisBlocksConference(blockToDelete,crsAppUserId);
		
		em.remove(blockToDelete);
	}
	
	private void verifyUserIdHasAccessToModifyThisBlocksConference(BlockEntity blockToUpdate, UUID crsAppUserId) throws UnauthorizedException
	{
